...
public class OversightAgency extends Agency {
	...
	/**
	 * Reads a BatchedTelecomResponse from the Leader and processes it.
	...
	private boolean readResponseFromLeader(int distance)
		...
		BatchedTelecomResponse[] batchedResponses =
				(BatchedTelecomResponse[]) leaderIStream.readObject();
		for (int telecomId = 0; telecomId < numTelecoms; telecomId++) {
			if (batchedResponses[telecomId] == null) {
				continue;
			}
			BatchedTelecomResponse batchedResponse = batchedResponses[telecomId];
			for (TelecomResponse telecomResponse :
				batchedResponse.getTelecomResponses()) {
				processTelecomResponse(telecomResponse, distance);
			}
		}
		// Get the degree of the initial target for timing purposes
		if (distance == 0) {
			// There is only one BatchedTelecomReponse with one TelecomResponse in
			// it, in this case.
			int initialOwner;
			for (initialOwner = 0; initialOwner < numTelecoms; initialOwner++) {
				if (batchedResponses[initialOwner] != null) {
					break;
				}
			}
			targetDegree = batchedResponses[initialOwner].getTelecomResponses()[0].
					getTelecomRecords().length;
		}
		return true;
	}
